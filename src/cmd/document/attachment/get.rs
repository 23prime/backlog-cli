...
pub struct DocumentAttachmentGetArgs {
    document_id: String,
    attachment_id: u64,
    output: Option<PathBuf>,
}

impl DocumentAttachmentGetArgs {
    pub fn new(document_id: String, attachment_id: u64, output: Option<PathBuf>) -> Self {
        Self {
            document_id,
            attachment_id,
            output,
...
pub fn get(args: &DocumentAttachmentGetArgs) -> Result<()> {
    let client = BacklogClient::from_config()?;
    get_with(args, &client)
}

pub fn get_with(args: &DocumentAttachmentGetArgs, api: &dyn BacklogApi) -> Result<()> {
    let (bytes, filename) =
        api.download_document_attachment(&args.document_id, args.attachment_id)?;
...
mod tests {
    ...
    fn args(output: Option<PathBuf>) -> DocumentAttachmentGetArgs {
        DocumentAttachmentGetArgs::new("abc123".to_string(), 1, output)
    }
